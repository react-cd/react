...
function bailoutOnAlreadyFinishedWork(
  ...
  if (childExpirationTime < renderExpirationTime) {  // 说明子节点也没有在这次渲染里面需要完成的
    // 首先这是一个非常大的优化了，有了 childExpirationTime就可以很快的跳过 子节点的
    // 如果   没有这个的话，还需要遍历到子节点才能知道 是否有更新
    // 个人感觉，算个 局部优化
 ...
 * 判断  组件是否 可以优化
 * 根据节点类型分发处理 每种 类型的 fiber节点 更新方式不同
 * 根据 expirationTime  等信息 判断是否 可以 跳过
 * 就是执行对整棵数每个节点进行更新的操作
 */
function beginWork(
  current: Fiber | null,
  workInProgress: Fiber,
  renderExpirationTime: ExpirationTime, // rootFiber.nextExpirationTimeToWorkOn
): Fiber | null {
  // renderExpirationTime  作用 标志这次渲染的时候 优先级 最大到哪个节点  
  //  expirationTime  对应的是这个节点的产生更新的过期时间，
  // 但是对于 rootFiber而言 ，只有调用ReactDom.render才会更新，后期 不会更新了 
  //页面渲染完成之后 比如 一个组件  点击一个按钮  setState更新组件 肯定是组件上的更新  不会更新到rootFiber上
  // 这个是 fiber节点的 expirationTime 
  const updateExpirationTime = workInProgress.expirationTime;
  // 需要做 一些优化       
  // 一个 节点跟新，不可能会引起其他的 兄弟节点的更新
  if (current !== null) {
    ...
    if (
      oldProps === newProps &&
      !hasLegacyContextChanged() &&
      updateExpirationTime < renderExpirationTime // 优先级 大就进行渲染，优先级就 这次就不用渲染，这个节点是可以跳过的     可以使用setTimeout 进行尝试 
      // 有更新但是优先级不高
    ) {
