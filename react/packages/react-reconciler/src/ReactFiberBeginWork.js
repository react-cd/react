...
function bailoutOnAlreadyFinishedWork(
  ...
  if (
    ...
    childExpirationTime > renderExpirationTime //  这次渲染子节点 也没有需要更新的  直接返回null
  ) {
...
function beginWork(
  ...
  // 如果节点上expirationTime 大于 renderExpirationTime  说明  
  // 需要做 一些优化       
  ...
  if (current !== null) {
    ...
    if (
      ...
      (updateExpirationTime === NoWork || // 没有更新
        updateExpirationTime > renderExpirationTime) // 说明优先级比较低 ，
    ) {
      // / 优先级 大就进行渲染，优先级就 这次就不用渲染，这个节点是可以跳过的     可以使用setTimeout 进行尝试 
      // 有更新但是优先级不高
